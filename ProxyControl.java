...
public class ProxyControl extends GenericController {
    ...
    private static final String TARGET_CATEGORY = "REQUEST_LOGGING";
    private static final Logger logToFile = LoggingManager.getLoggerFor(TARGET_CATEGORY);
    private static int logFilePrefix = 0;
    private static String logSessionDir = "";
   ...
   // private static final String ASSERTION_GUI = AssertionGui.class.getName();
    ...
    public static void setSessionDir(String dirName){
    	logSessionDir = dirName;
    	new File(logSessionDir).mkdir();
    }
    ...
    public synchronized void logRawdata(final HttpRequestHdr request, final HTTPSamplerBase sampler, final SampleResult result){
    	 
    	if (sampler != null) {
			if (filterContentType(result) && filterUrl(sampler)) {
			     // set logging for HTTP(S) row data to separate file
				
				// create folder for session
				if (logSessionDir.equals("") ){
					logSessionDir = "session_" + new Date().getTime();
					new File(logSessionDir).mkdir();
				}
				
	      		try {
	      			logFilePrefix++;
	     			FileWriter requestWriter = new FileWriter(logSessionDir + File.separator + "request_" + logFilePrefix + ".txt");
	     			WriterTarget logTarget = new WriterTarget(requestWriter, new RawFormatter());
	     	    	LoggingManager.setTarget(logTarget, TARGET_CATEGORY);
	     	    	logToFile.info(new String(request.getRawRequestData()));
	     	    	
	     	    	FileWriter responsetWriter = new FileWriter(logSessionDir + File.separator + "response_" + logFilePrefix + ".txt");
	     	    	WriterTarget logTargetResponse = new WriterTarget(responsetWriter, new RawFormatter());
	     	    	LoggingManager.setTarget(logTargetResponse, TARGET_CATEGORY);
	     	    	logToFile.info(new String(result.getResponseHeaders()));
    ...
    public synchronized void deliverSampler(final HTTPSamplerBase sampler, final TestElement[] subConfigs, final SampleResult result) {
       // boolean notifySampleListeners = true;
    	boolean notifySampleListeners = false;
        if (sampler != null) {
...
//            });
    ...
    private boolean matchesPatterns(String url, CollectionProperty patterns) {
    	...
    	PropertyIterator i = patterns.iterator();
    	
    	while(i.hasNext()){
            
    		String item = i.next().getStringValue();
